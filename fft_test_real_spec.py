...
def plot_spectrum(fpga, bram_names, cx=True, num_acc_updates=None):
    ...
    full_scale = 108.6  # manually determined limit of the the plot. differs from time to time, recalibrate 
    ...
    for i, bram_name in enumerate(bram_names):
        ...
        spectrum_dBFS = 10 * np.log10((spectrum/acc_len) + 1) - full_scale
        faxis = fbins * df
...
#   spectra = np.zeros((num_points, num_brams))  # Array to hold spectra from all BRAMs
#   for i, bram_name in enumerate(bram_names):
#        print(bram_name)
#        row = i // 2
#        col = i % 2
...
#        # Prepare file name with BRAM name
#        date = datetime.datetime.now().strftime('%Y_%m_%d')
#        time = datetime.datetime.now().strftime('%H_%M_%S')
#        file_name = '/data_' +bram_name+'_'+time+'.txt'
#        directory = 'data_'+date

#        # Create directory if it doesn't exist
#        if not os.path.exists('data_'+date):
#            os.mkdir('data_'+date)
#            if not os.path.exists(directory+'/'+file_name):
#               np.savetxt(directory+file_name, (spectrum_dBFS))
#               print(file_name)        
#        else:
#            if not os.path.exists(directory+'/'+file_name):
#               np.savetxt(directory+file_name, (spectrum_dBFS))
#               print(file_name) 
#        # Save spectrum data for each BRAM block
...
#    print(diff_spectra)
    ...
    def update(frame, *fargs):
        ...
        for i, bram_name in enumerate(bram_names):
            ...
            date=datetime.datetime.now().strftime('%Y_%m_%d')
            time=datetime.datetime.now().strftime('%H_%M_%S')
            file_name='/data'+time+bram_names[i]+'.txt'
            directory='data_'+date    
...
if __name__ == "__main__":
    ...
    if opts.fpgfile != '':
        ...
        if mode == 1:
            fpg_prebuilt = '/home/jasper/Desktop/Final_version_spectrometer/Real/fft_test_real_spec/outputs/fft_test_real_spec_2024-09-24_1155.fpg'
        else:
            fpg_prebuilt = '/home/jasper/Desktop/Final_version_spectrometer/Real/fft_test_real_spec/outputs/fft_test_real_spec_2024-09-24_1155.fpg'
